...
void startwifi() {
	...
	if (kernel_module_loaded((char *)"dhd") != 0) {
		writelog(insmod("/lib/modules/dhd.ko", "firmware_path=/data/data/android.tether/bin/rtecdc.bin nvram_path=/etc/nvram.txt"),(char *)"Loading dhd.ko module");
	}
	...
	if (file_exists((char *)"/data/data/android.tether/conf/wpa_supplicant.conf") == 0) {
		...
		writelog(system("chmod 0777 /data/local/tmp;cd /data/local/tmp;wpa_supplicant -B -f -Dwext -ieth0 -c/data/data/android.tether/conf/wpa_supplicant.conf"),
...
void startdnsmasq() {
    // Starting dnsmasq
	writelog(system("/data/data/android.tether/bin/dnsmasq -i eth0 --resolv-file=/data/data/android.tether/conf/resolv.conf --conf-file=/data/data/android.tether/conf/dnsmasq.conf"),(char*)"Starting dnsmasq");
}
...
void readlanconfig() {
	if (file_exists((char*)"/data/data/android.tether/conf/tether.conf") == 0) {
		FILE *lanconf;
		char buffer[100];
		char name[50];
		char value[50];
		if (!(lanconf = fopen("/data/data/android.tether/conf/tether.conf", "r")) ) {
			fprintf(stderr, "Can't open /data/data/android.tether/conf/tether.conf for read \n");
			return;
		}
...
void readwificonfig() {
	if (file_exists((char*)"/data/data/android.tether/conf/tether.conf") == 0) {
		...
		char name[20];
		char value[20];
		if (!(wificonf = fopen("/data/data/android.tether/conf/tether.conf", "r")) ) {
			fprintf(stderr, "Can't open /data/data/android.tether/conf/tether.conf for read \n");
			return;
		}
