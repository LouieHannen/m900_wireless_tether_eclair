...
public class MainActivity extends Activity {
	...
	private TextView totalDateText = null;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		this.totalText = (TextView) findViewById(R.id.trafficTotal);
		this.totalDateText = (TextView) findViewById(R.id.trafficTotalDate);
		...
		this.startBtnListener = new OnClickListener() {
			public void onClick(View v) {
				...
				new Thread(new Runnable() {
					public void run() {
						boolean started = MainActivity.this.application
								.startTether();
						MainActivity.this
						...
						if (started != true) {
							...
							if (Integer.parseInt(Build.VERSION.SDK) >= Build.VERSION_CODES.ECLAIR) {
								boolean bluetoothPref = MainActivity.this.application.settings
										.getBoolean("bluetoothon", false);
								if (bluetoothPref) {
									boolean bluetoothDiscoverable = MainActivity.this.application.settings
											.getBoolean(
							...
							String wifiStatus = MainActivity.this.application.coretask
									.runShellCommand("sh", "stdout",
	...
	public boolean onTrackballEvent(MotionEvent event) {
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			String tetherStatus = this.application.coretask.runShellCommand(
					"sh", "stdout", "getprop tether.status");
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		if (this.application.accessControlSupported) {
			SubMenu accessctr = menu.addSubMenu(0, MENU_ACCESS, 0,
					getString(R.string.accesscontroltext));
			accessctr.setIcon(drawable.ic_menu_manage);
		}
		SubMenu log = menu.addSubMenu(0, MENU_LOG, 0,
				getString(R.string.logtext));
		log.setIcon(drawable.ic_menu_agenda);
		SubMenu reset = menu.addSubMenu(0, MENU_RESET, 0,
				getString(R.string.resettext));
		reset.setIcon(drawable.ic_menu_close_clear_cancel);
	...
	private BroadcastReceiver intentReceiverc = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(Intent.ACTION_BATTERY_CHANGED)) {
				...
				batteryTemperature.setText("" + (temp / 10)
						+ getString(R.string.temperatureunit1));
			}
		}
	};
	...
	private BroadcastReceiver intentReceiverf = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(Intent.ACTION_BATTERY_CHANGED)) {
				int temp = (intent.getIntExtra("temperature", 0)) + 5;
				batteryTemperature.setText("" + ((((temp / 10) * 9) / 5) + 32)
						+ getString(R.string.temperatureunit2));
			}
		}
	};
		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_CHECK_LOG:
				...
				MainActivity.this.application
						.displayToastMessage("Tethering started with errors! Please check 'Show log'.");
			...
			case MESSAGE_CANT_START_TETHER:
				...
				MainActivity.this.application
						.displayToastMessage("Unable to start tethering. Please try again!");
				MainActivity.this.toggleStartStop();
				break;
			...
			case MESSAGE_TRAFFIC_COUNT:
				...
				long uploadTotal = ((TetherApplication.DataCount) msg.obj).historyUp;
				long downloadTotal = ((TetherApplication.DataCount) msg.obj).historyDown;
				Date historyDate = ((TetherApplication.DataCount) msg.obj).historyDate;
				...
				MainActivity.this.uploadText.setText(MainActivity.this
						.formatCount(uploadTraffic, false));
				MainActivity.this.totalText.setText(MainActivity.this
						.formatCount(uploadTotal + downloadTotal, false));
				MainActivity.this.downloadText.setText(MainActivity.this
						.formatCount(downloadTraffic, false));
				MainActivity.this.totalText.invalidate();
				MainActivity.this.downloadText.invalidate();
				MainActivity.this.uploadText.invalidate();

				MainActivity.this.uploadRateText.setText(MainActivity.this
						.formatCount(uploadRate, true));
				MainActivity.this.totalDateText
						.setText(getString(R.string.sincetext) + " "
								+ formatDate(historyDate));
				MainActivity.this.downloadRateText.setText(MainActivity.this
						.formatCount(downloadRate, true));
				MainActivity.this.downloadRateText.invalidate();
				MainActivity.this.totalDateText.invalidate();
				MainActivity.this.uploadRateText.invalidate();
				MainActivity.this.application.preferenceEditor.putLong(
						"historydate", historyDate.getTime());
				MainActivity.this.application.preferenceEditor.putLong(
						"historyup", uploadTotal);
				MainActivity.this.application.preferenceEditor.putLong(
						"historydown", downloadTotal);
			...
			case MESSAGE_DOWNLOAD_STARTING:
				...
				MainActivity.this.downloadUpdateLayout
						.setVisibility(View.VISIBLE);
				break;
			case MESSAGE_DOWNLOAD_PROGRESS:
				MainActivity.this.progressBar.setIndeterminate(false);
				MainActivity.this.progressText.setText(msg.arg1 + "k /"
						+ msg.arg2 + "k");
				MainActivity.this.progressBar.setProgress(msg.arg1 * 100
						/ msg.arg2);
				break;
			...
			case MESSAGE_DOWNLOAD_BLUETOOTH_FAILED:
				...
				MainActivity.this.startBtn.setClickable(true);
				MainActivity.this.application.preferenceEditor.putBoolean(
						"bluetoothon", false);
				MainActivity.this.application.preferenceEditor.commit();
	...
	private boolean isThetherRunning() {
	...
	private void toggleStartStop() {
		...
		if ((dnsmasqRunning == true && natEnabled == true)
			...
			String tetherMode = this.application.coretask.runShellCommand("sh",
					"stdout", "getprop tether.mode");
			String tetherStatus = this.application.coretask.runShellCommand(
					"sh", "stdout", "getprop tether.status");
			if (tetherStatus.equals("running")) {
				if (!(tetherMode.equals("wifi") == true || tetherMode
						.equals("bt") == true)) {
					MainActivity.this.application
							.displayToastMessage("Wired-tethering seems to be running at the moment. Please disable it first!");
				}
			}

			// Checking, if cyanogens usb-tether is currently running
			tetherStatus = this.application.coretask.runShellCommand("sh",
					"stdout", "getprop tethering.enabled");
			if (tetherStatus.equals("1")) {
				MainActivity.this.application
						.displayToastMessage("USB-tethering seems to be running at the moment. Please disable it first: Settings -> Wireless & network setting -> Internet tethering.");
			}
	...
	private String formatDate(Date date) {
		String dateStr = DateFormat.getDateInstance(DateFormat.SHORT).format(
				date);
		return dateStr;
	}
				...
				.setNeutralButton("Ignore",
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
								...
								MainActivity.this.application.installFiles();
								MainActivity.this.application
										.displayToastMessage("Access Control disabled.");
							}
						}).show();
	}
				...
				.setNeutralButton("Donate",
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
								...
								Uri uri = Uri
										.parse(getString(R.string.paypalUrl));
	...
	private void openResetDialog() {
		LayoutInflater li = LayoutInflater.from(this);
		View view = li.inflate(R.layout.resetview, null);
		if (!isThetherRunning()) {
			new AlertDialog.Builder(MainActivity.this)
					.setTitle("Reset")
					.setView(view)
			...
			TextView tv = (TextView) view.findViewById(R.id.resetTextView);
			tv.setText(getString(R.string.resetblock));
			new AlertDialog.Builder(MainActivity.this)
					.setTitle("Reset")
					.setView(view)
	...
	private void resetData() {
		this.application.preferenceEditor.putLong("historydate",
				new java.util.Date().getTime());
		this.application.preferenceEditor.putLong("historyup", 0);
		this.application.preferenceEditor.putLong("historydown", 0);
			...
			dialog.setNegativeButton("Yes",
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog,
							...
							if (debug.exists())
								Log.d(MSG_TAG, "Yes pressed");
							MainActivity.this.application.downloadUpdate(
